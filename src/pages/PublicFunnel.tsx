...
export default function PublicFunnel() {
  ...
  const processNextBlock = async () => {
    ...
    if (block.type === "input") {
      ...
      setTyping(false);
      setMessages(prev => [...prev, { type: "bot", content: block.content, blockType: "input" }]);
    ...
    if (block.type === "buttons") {
      ...
      await new Promise(r => setTimeout(r, 800));
      setTyping(false);
      setMessages(prev => [...prev, { type: "bot", content: block.content, blockType: "buttons" }]);
          ...
          <div key={i} className={`flex ${msg.type === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${
              msg.type === "user"
                ...
                : "bg-[#1f2c34] text-[#e9edef] rounded-bl-none"
            }`}>
    ...
    case "image":
      return (
        <div>
          ...
          {c.caption && <p className="text-xs text-[#8696a0]">{replaceVars(c.caption)}</p>}
        </div>
      );
    case "video":
      return (
          ...
          <div className="flex flex-col gap-1 mt-1">
            {(c.buttons ?? []).map((btn: any, i: number) => (
              <button
                ...
                className="text-left px-3 py-2 rounded bg-[#2a3942] hover:bg-[#3a4a52] text-[#00a884] text-sm font-medium transition-colors"
              >
                {btn.label}
              </button>
            ))}
